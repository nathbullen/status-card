...
// Custom interfaces for status-card-plus
...
export function deviceClassItems(domain: string): string[] {
  return deviceClasses[domain] || [];
}

export function getAllowedTypes(contentArray: string[] | undefined): string[] {
  if (!contentArray || contentArray.length === 0) {
    return [];
  }
  const types: string[] = [];
  contentArray.forEach((contentItem) => {
    const [domain, deviceClass] = contentItem.split(".");
    types.push(domain);
    if (deviceClass) {
      types.push(...deviceClassItems(domain));
    }
  });
...
// Re-export from ha module
export { caseInsensitiveStringCompare };

// Export types from ha module
export type { AreaRegistryEntry, EntityRegistryEntry, DeviceRegistryEntry };
